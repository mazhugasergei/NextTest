...
export default ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    <div className="absolute inset-0 isolate grid place-items-center h-full bg-[#000000cc] backdrop-blur p-4 pt-[4.5rem]">
      <button
        onClick={router.back}
      ...
      <div
        className={`overflow-auto w-full max-h-full bg-[#222] border-[0.0625rem] border-[#444] rounded-lg p-8 ${className}`}
      >
        {children}
      </div>
    </div>
